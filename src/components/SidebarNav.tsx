  ...
  return (
    <>
      ...
      >
        ...
        <button
          onClick={onToggle}
          className="absolute -right-4 top-1/2 -translate-y-1/2 z-50 w-8 h-8 rounded-full bg-white/10 border border-white/20 flex items-center justify-center hover:bg-white/20 transition-all duration-300 shadow-lg"
            ...
            <div className="space-y-0.5">
              {categories.map((cat, index) => {
                ...
                return (
                  ...
                  >
                    {/* Hover background */}
                    <div className={`absolute inset-0 bg-white/[0.03] opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isActive ? 'opacity-100' : ''}`} />
                    ...
                    >
                      ...
                      {isActive && (
                        <motion.div
                          ...
                          className="absolute left-0 top-1/2 -translate-y-1/2 w-[2px] h-4 bg-white/40 rounded-r-full"
                      ...
                      <AnimatePresence mode="wait">
                        {isCollapsed ? (
                          <motion.span
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className={`text-[10px] font-mono ${isActive ? 'text-white' : 'text-white/40'} group-hover:text-white/60 transition-colors`}
                          >
                          ...
                          >
                            <span className={`text-[10px] font-mono ${isActive ? 'text-white/50' : 'text-white/20'}`}>
                              {String(index + 1).padStart(2, "0")}
                            </span>
                            <span className={`text-xs tracking-wide ${isActive ? 'text-white/80' : 'text-white/40'} group-hover:text-white/60 transition-colors`}>
                              {cat.name}
                            </span>
                          </motion.div>
                        )}
